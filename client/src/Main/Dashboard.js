...
const Dashboard = ({ reactRoot }) => {
    ...
    const getUser = async () => {
        try {
            const { data } = await axios.get('http://localhost:6005/user/info', { withCredentials: true });
            setUser(data.user);
    ...
    const handleDeleteAccount = async () => {
        try {
            await axios.delete(`http://localhost:6005/user/delete/${user._id}`);
    ...
    const handleSave = async () => {
        try {
            const response = await axios.put('http://localhost:6005/update-username', {
                userId: user._id,
